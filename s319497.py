...
def solution(p: Problem):
    ...
    # Inizializzazione sicura: definiamo le variabili LNS a zero/False per evitare NameError.
    USE_LNS = False
    LNS_PCT = 0.0
    LNS_PROB = 0.0  
    ...
    if beta <= 1.0:
        ...
        # Usiamo parametri standard robusti per favorire l'ottimizzazione del percorso.
        ELITISM = 5
        
        if IS_HUGE:
            # Parametri conservativi per istanze giganti (N >= 500) per stare nei tempi
            MU = 30
            LAMBDA = 60
            MAX_GENERATIONS = 300
            EARLY_STOP_LIMIT = 50
            ...
            if beta == 1.0:
                # Attiviamo il Large Neighborhood Search (LNS) solo sul caso strettamente lineare
                USE_LNS = True
                LNS_PCT = 0.20
                LNS_PROB = 0.5
                
        else:
            # Parametri aggressivi per istanze normali (esplorazione profonda)
            MU = 50
            LAMBDA = 100
        ...
        # Riduciamo l'esplorazione geometrica (LNS tenuto a False) e scaliamo dinamicamente 
        # la popolazione per concentrarci sull'ordine di scarico dei nodi pesanti.
        MU = int(max(10, N * 0.25))
        LAMBDA = MU * 2
    ...
    # PULIZIA DEI TIPI NUMPY
    # Convertiamo forzatamente ogni nodo e quantità d'oro in int() Python nativi
    clean_final_path = [(int(node), int(gold)) for node, gold in final_path]
    
    return clean_final_path